...
class GraphPanel extends JPanel{
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (!(parentFrame.getFlag() == '-'))
        {
        	if (parentFrame.getFlag() == 'l') {
        		g1.setColor(Color.red);
                Polygon p1 = new Polygon();
                for (int x = -600; x <= 600; x++) {
                    p1.addPoint(w+scale*x, h - scale*((parentFrame.getA()*x) + parentFrame.getB()));
                }
                g1.drawPolyline(p1.xpoints, p1.ypoints, p1.npoints);
        	}
        	if (parentFrame.getFlag() == 'k') {
        		g1.setColor(Color.green);
                Polygon p2 = new Polygon();
                for (int x = -600; x <= 600; x++) {
                    p2.addPoint(w+scale*x, h - scale*((parentFrame.getA() * x * x) + (parentFrame.getB() * x ) + parentFrame.getC()));
                }
